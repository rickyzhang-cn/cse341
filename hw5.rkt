        ...
        [(ifnz? e)
         (let ([v (eval-under-env (ifnz-e1 e) env)])
           (if (int? v)
               ...
               (error "MUPL ifnz e1 should be int")))]
        [(fun? e) 
         (if (null? (fun-nameopt e))
             (closure env e)
             (closure (envextend env (cons (fun-nameopt e) e)) e))]
        [(call? e)
         (let ([v1 (eval-under-env (call-funexp e) env)]
               [v2 (eval-under-env (call-actual e) env)])
           (if (closure? v1)
               (let ([e (closure-env v1)]
                     [f (closure-fun v1)])
                 (eval-under-env (fun-body f) (envextend e (cons (fun-formal f) v2))))
               (error "MUPL call applied to non-closure")))]
        [(mlet? e)
         (let ([v (eval-under-env (mlet-e e) env)])
           (eval-under-env (mlet-body e) (envextend env (cons (mlet-var e) v))))]
        [(apair? e)
